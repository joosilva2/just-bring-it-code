...
async function sendToUtmify(token: string, data: any) {
  ...
  const utmifyPayload = {
    ...
    commission: {
      totalPriceInCents: data.amount || 8240,
      gatewayFeeInCents: Math.round((data.amount || 8240) * 0.06) + 197,
      userCommissionInCents:
        (data.amount || 8240) -
        (Math.round((data.amount || 8240) * 0.06) + 197),
      currency: "BRL",
    },
  };
